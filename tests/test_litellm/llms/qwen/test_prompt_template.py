...
def test_qwen_prompt_modifier_auto_mode():
    messages = [{"role": "user", "content": "Hello"}]
    modified_messages = qwen_thinking_prompt_modifier(messages, "auto")
    assert modified_messages == messages

def test_qwen_prompt_modifier_enable_empty_think_tags():
    messages = [{"role": "user", "content": "What is AI?"}]
    modified_messages = qwen_thinking_prompt_modifier(messages, "enable_empty_think_tags")
    assert len(modified_messages) == 2
    assert modified_messages[1] == {"role": "assistant", "content": "<think>\n</think>"}
    # Ensure original message is untouched
    assert modified_messages[0] == messages[0]

def test_qwen_prompt_modifier_force_think_last_user():
    messages = [{"role": "user", "content": "Tell me a story."}]
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_think")
    assert modified_messages[0]["content"] == "/think\nTell me a story."

def test_qwen_prompt_modifier_force_no_think_last_user():
    messages = [{"role": "user", "content": "Just give me the facts."}]
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_no_think")
    assert modified_messages[0]["content"] == "/no_think\nJust give me the facts."

def test_qwen_prompt_modifier_force_think_multiturn_last_user():
    messages = [
    ...
    assert modified_messages[0]["content"] == "Hello" # Previous messages untouched

def test_qwen_prompt_modifier_force_think_last_is_assistant():
    messages = [
        {"role": "user", "content": "Initial query"},
        {"role": "assistant", "content": "My response"}
    ]
    # Current logic: if last message is not user, 'force_think' on last user message
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_think")
    assert modified_messages[0]["content"] == "/think\nInitial query"
    assert modified_messages[1]["content"] == "My response"

def test_qwen_prompt_modifier_force_think_no_user_message():
    messages = [{"role": "assistant", "content": "I am ready."}]
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_think")
    assert modified_messages == messages # No change as no user message to modify

def test_qwen_prompt_modifier_empty_messages_list():
    messages = []
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_think")
    assert modified_messages == []

def test_qwen_prompt_modifier_force_think_multimodal_user_content_first_text():
    messages = [
        {"role": "user", "content": [
...
def test_qwen_prompt_modifier_force_think_multimodal_user_content_image_first():
    messages = [
        {"role": "user", "content": [
            {"type": "image_url", "image_url": {"url": "..."}},
    ...
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_think")
    assert modified_messages[0]["content"][1]["type"] == "text"
    assert modified_messages[0]["content"][1]["text"] == "/think\nWhat do you see?"

def test_qwen_prompt_modifier_force_think_multimodal_user_content_only_image():
    messages = [
        {"role": "user", "content": [
            {"type": "image_url", "image_url": {"url": "..."}}
        ]}
    ...
    modified_messages = qwen_thinking_prompt_modifier(messages, "force_think")
    # Expects a new text part to be added
    assert len(modified_messages[0]["content"]) == 2
    assert modified_messages[0]["content"][0]["type"] == "text"
    assert modified_messages[0]["content"][0]["text"] == "/think" # .strip() was used

def test_qwen_prompt_modifier_original_messages_not_mutated():
    messages = [{"role": "user", "content": "Hello"}]
    messages_copy = [{"role": "user", "content": "Hello"}] # Deep copy for assertion

    qwen_thinking_prompt_modifier(messages, "force_think")
    assert messages == messages_copy, "Original messages list should not be mutated."

    qwen_thinking_prompt_modifier(messages, "enable_empty_think_tags")
    assert messages == messages_copy, "Original messages list should not be mutated after empty tags."
